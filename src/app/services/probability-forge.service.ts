...
export class ProbabilityForgeService {
  ...
  // Token generation rate based on discoveries
  ...
  // Pull cost based on discoveries
  ...
  checkUnlock(totalQuantaGenerated: number): void {
    ...
    // Require both 1Qi Quanta AND all three artifact branches at tier 10
  ...
  generateFateTokens(deltaTime: number): void {
    ...
    // Base 1 token/second + 2% per discovered outcome
  ...
  // Calculate current probabilities with all modifiers
  private calculateProbabilities(): Map<OutcomeRarity, number> {
    ...
    // Apply exponential scaling (mythic chance increases with mythic count)
  ...
  pull(): PullResult {
    ...
    // Select outcome from that rarity tier
  ...
  private checkPity(): OutcomeRarity | null {
    ...
    // Similar checks for epic and rare pity could be added
  ...
  private selectOutcome(rarity: OutcomeRarity): ProbabilityOutcome {
    ...
    // Apply duplicate protection if unlocked
    ...
    if (dupeProtection?.unlocked) {
      ...
      if (newOutcomes.length > 0) {
        // Weighted selection favoring new outcomes
  ...
  // Unlock and upgrade fate weights
